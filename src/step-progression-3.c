...
struct fv_app_step_progression3_t
{
...
{
  struct fv_app_step_progression3_t *app = (struct fv_app_step_progression3_t *) data;
  four_square_to_nine_number_step_progression3 (progression, one, two, three, four);
  int count = count_squares_in_step_progression3 (progression);
  if (count > 4)
...
{
  struct fv_app_step_progression3_t *app = (struct fv_app_step_progression3_t *) state->input;
  char *end = NULL;
  switch (key)
    {
    case 'I':
...
int
fituvalu_step_progression3 (struct fv_app_step_progression3_t *app)
{
  if (app->instream && !app->in_binary)
    read_square_and_run (app->instream,
                         fwd_4sq_progression4,
                         check_progression,
    ...
    fwd_4sq_progression4 (app->oneshot, app->start, app->finish, app->incr,
                          check_progression, app);
  else
    loop_and_run (fwd_4sq_progression4,
                  check_progression,
                  app->start, app->finish, app->incr, app);
...
{
  struct fv_app_step_progression3_t app;
  ...
  return fituvalu_step_progression3 (&app);
}
