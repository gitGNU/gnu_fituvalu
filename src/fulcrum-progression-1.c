...
struct fv_app_fulcrum_progression1_t
{
...
{
  struct fv_app_fulcrum_progression1_t *app = (struct fv_app_fulcrum_progression1_t *) data;
  four_square_to_nine_number_fulcrum_progression1 (progression, one, two, three, four);
  int count = count_squares_in_fulcrum_progression1 (progression);
  if (count > 4)
...
{
  struct fv_app_fulcrum_progression1_t *app = (struct fv_app_fulcrum_progression1_t *) state->input;
  char *end = NULL;
  switch (key)
    {
    case 'I':
...
int
fituvalu_fulcrum_progression1 (struct fv_app_fulcrum_progression1_t *app)
{
  if (app->instream && !app->in_binary)
    read_square_and_run (app->instream,
                         fwd_4sq_progression3,
                         check_progression,
    ...
    fwd_4sq_progression3 (app->oneshot, app->start, app->finish, app->incr,
                          check_progression, app);
  else
    loop_and_run (fwd_4sq_progression3,
                  check_progression,
                  app->start, app->finish, app->incr, app);
...
{
  struct fv_app_fulcrum_progression1_t app;
  memset (&app, 0, sizeof (app));
  app.incr = 1;
  app.display_record = display_nine_record;
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_fulcrum_progression1 (&app);
}
