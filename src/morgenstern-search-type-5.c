...
struct fv_app_morgenstern_search_type_5_t;
static int
...
static void
search_type_5 (mpz_t m1, mpz_t n1, mpz_t m2, mpz_t n2, void *data)
{
  static int inited = 0;
  static mpz_t a[3][3];
  ...
  struct fv_app_morgenstern_search_type_5_t *app = (struct fv_app_morgenstern_search_type_5_t *) data;
...
*/
 ...
 // C = YX1sum * Z2;
 // D = YX1sum * YX2dif;
  mpz_mul (a[0][0], z1, yx2sum);
  mpz_mul (a[0][0], a[0][0], a[0][0]);

  mpz_mul (a[1][2], yx1dif, yx2sum);
  mpz_mul (a[1][2], a[1][2], a[1][2]);

  mpz_mul (a[2][1], yx1sum, yx2sum);
  mpz_mul (a[2][1], a[2][1], a[2][1]);

  mpz_mul (a[0][2], yx1sum, z2);
  mpz_mul (a[0][2], a[0][2], a[0][2]);

  mpz_mul (a[1][0], yx1sum, yx2dif);
  ...
  // I^2 = D^2 + A^2 - H^2;
  // E^2 = G^2 + I^2 - B^2;
  mpz_add (a[0][1], a[1][0], a[1][2]);
  mpz_sub (a[0][1], a[0][1], a[2][1]);

  mpz_add (a[2][0], a[0][2], a[1][2]);
  mpz_sub (a[2][0], a[2][0], a[2][1]);

  mpz_add (a[2][2], a[1][0], a[0][0]);
  mpz_sub (a[2][2], a[2][2], a[2][1]);

  mpz_add (a[1][1], a[2][0], a[2][2]);
  mpz_sub (a[1][1], a[1][1], a[0][1]);
...
int
fituvalu_morgenstern_search_type_5 (struct fv_app_morgenstern_search_type_5_t *app, FILE *in)
{
  if (!app->infile)
    {
      if (app->in_binary)
        morgenstern_search_from_binary (app->max, in, search_type_5, app);
      else
        morgenstern_search (app->max, in, search_type_5, app);
    }
  else
    {
      if (app->in_binary)
        {
          if (app->inmem)
            morgenstern_search_dual_binary_mem (in, app->infile, search_type_5, app);
          else
            morgenstern_search_dual_binary (in, app->infile, search_type_5, app);
        }
      else
        {
          if (app->inmem)
            morgenstern_search_dual_mem (in, app->infile, search_type_5, app);
          else
            morgenstern_search_dual (in, app->infile, search_type_5, app);
        }
    }
  return 0;
}
...
{
  struct fv_app_morgenstern_search_type_5_t *app = (struct fv_app_morgenstern_search_type_5_t *) state->input;
  switch (key)
    {
    case 'm':
      app->inmem = 1;
...
{
  struct fv_app_morgenstern_search_type_5_t app;
