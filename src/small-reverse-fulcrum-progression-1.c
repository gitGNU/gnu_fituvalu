...
struct fv_app_small_reverse_fulcrum_progression1_t
{
  FILE *instream;
...
{
  struct fv_app_small_reverse_fulcrum_progression1_t *app = (struct fv_app_small_reverse_fulcrum_progression1_t *) data;
  progression[5] = one;
  progression[4] = two;
  progression[3] = three;
  progression[2] = four;
  int count = 4;
  unsigned long long e1 = progression[5] - progression[4];
  unsigned long long f1 = progression[3] - progression[2];
  unsigned long long d1 = e1 + f1;
  progression[6] = progression[5] + f1;
  progression[7] = progression[6] + e1;
  progression[8] = progression[7] + d1;

  progression[1] = progression[2] - e1;
...
{
  struct fv_app_small_reverse_fulcrum_progression1_t *app = (struct fv_app_small_reverse_fulcrum_progression1_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_reverse_fulcrum_progression1 (struct fv_app_small_reverse_fulcrum_progression1_t *app)
{
  if (app->instream)
    small_read_square_and_run (app->instream,
                               small_rev_4sq_progression1,
                               check_progression,
                               app->start, app->finish, app);
  else
    small_loop_and_run (small_rev_4sq_progression1,
                        check_progression,
                        app->start, app->finish, app);
  return 0;
}
...
{
  struct fv_app_small_reverse_fulcrum_progression1_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_reverse_fulcrum_progression1 (&app);
}
