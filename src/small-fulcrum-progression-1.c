...
struct fv_app_small_fulcrum_progression1_t
{
  FILE *instream;
...
{
  struct fv_app_small_fulcrum_progression1_t *app = (struct fv_app_small_fulcrum_progression1_t *) data;
  progression[0] = one;
  progression[1] = two;
  progression[2] = three;
  progression[3] = four;
  int count = 4;
  unsigned long long int d1 = progression[1] - progression[0];
...
{
  struct fv_app_small_fulcrum_progression1_t *app = (struct fv_app_small_fulcrum_progression1_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_fulcrum_progression1 (struct fv_app_small_fulcrum_progression1_t *app)
{
  if (app->instream)
    small_read_square_and_run (app->instream,
                               small_fwd_4sq_progression3,
                               check_progression,
                               app->start, app->finish, app);
  else
    small_loop_and_run (small_fwd_4sq_progression3,
                        check_progression,
                        app->start, app->finish, app);
...
{
  struct fv_app_small_fulcrum_progression1_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_fulcrum_progression1 (&app);
}
