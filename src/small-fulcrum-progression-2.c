...
struct fv_app_small_fulcrum_progression2_t
{
  FILE *instream;
...
{
  struct fv_app_small_fulcrum_progression2_t *app = (struct fv_app_small_fulcrum_progression2_t *) data;
  progression[1] = one;
  progression[2] = two;
  progression[3] = three;
  progression[4] = four;
  int count = 4;
  unsigned long long e1 = progression[2] - progression[1];
  unsigned long long f1 = progression[3] - progression[2];
  unsigned long long d1 = e1 + f1;
  progression[0] = progression[1] - d1;
  progression[5] = progression[4] + e1;
...
{
  struct fv_app_small_fulcrum_progression2_t *app = (struct fv_app_small_fulcrum_progression2_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_fulcrum_progression2 (struct fv_app_small_fulcrum_progression2_t *app)
{
  if (app->instream)
...
{
  struct fv_app_small_fulcrum_progression2_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_fulcrum_progression2 (&app);
}
