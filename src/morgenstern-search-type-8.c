...
static void
search_type_8 (mpz_t m1, mpz_t n1, mpz_t m2, mpz_t n2, FILE *out)
{
...
static int
morgenstern_search_type_8 (FILE *in, FILE *out)
{
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      mpz_init (a[i][j]);
  ...
  if (!infile)
    {
      if (in_binary)
        morgenstern_search_from_binary (max, in, search_type_8, out);
      else
        morgenstern_search (max, in, search_type_8, out);
    }
  else
    {
      if (in_binary)
        {
          if (inmem)
            morgenstern_search_dual_binary_mem (in, infile, search_type_8, out);
          else
            morgenstern_search_dual_binary (in, infile, search_type_8, out);
        }
      else
        {
          if (inmem)
            morgenstern_search_dual_mem (in, infile, search_type_8, out);
          else
            morgenstern_search_dual (in, infile, search_type_8, out);
        }
    }
...
main (int argc, char **argv)
{
  ...
  return morgenstern_search_type_8 (stdin, stdout);
}
