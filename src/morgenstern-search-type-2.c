...
static void
search_type_2 (mpz_t m1, mpz_t n1, mpz_t m2, mpz_t n2, FILE *out)
{
...
morgenstern_search_type_2 (FILE *in, FILE *out)
{
  ...
  if (!infile)
    {
      if (in_binary)
        morgenstern_search_from_binary (max, in, search_type_2, out);
      else
        morgenstern_search (max, in, search_type_2, out);
    }
  else
    {
      if (in_binary)
        {
          if (inmem)
            morgenstern_search_dual_binary_mem (in, infile, search_type_2, out);
          else
            morgenstern_search_dual_binary (in, infile, search_type_2, out);
        }
      else
        {
          if (inmem)
            morgenstern_search_dual_mem (in, infile, search_type_2, out);
          else
            morgenstern_search_dual (in, infile, search_type_2, out);
        }
    }
