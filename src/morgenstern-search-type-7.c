...
struct fv_app_morgenstern_search_type_7_t;
static int
...
static void
search_type_7 (mpz_t m1, mpz_t n1, mpz_t m2, mpz_t n2, void *data)
{
  static int inited = 0;
  static mpz_t a[3][3];
  ...
  struct fv_app_morgenstern_search_type_7_t *app = (struct fv_app_morgenstern_search_type_7_t *) data;
...
*/
  ...
  // A^2 = H^2 + I^2 - D^2;
  // E^2 = A^2 + G^2 - F^2;
  mpz_add (a[2][0], a[0][1], a[0][2]);
  mpz_sub (a[2][0], a[2][0], a[1][0]);

  mpz_add (a[1][2], a[0][1], a[2][1]);
  mpz_sub (a[1][2], a[1][2], a[1][0]);

  mpz_add (a[0][0], a[2][1], a[2][2]);
  mpz_sub (a[0][0], a[0][0], a[1][0]);

  mpz_add (a[1][1], a[0][0], a[2][0]);
  mpz_sub (a[1][1], a[1][1], a[1][2]);
...
int
fituvalu_morgenstern_search_type_7 (struct fv_app_morgenstern_search_type_7_t *app, FILE *in)
{
  if (!app->infile)
    {
      if (app->in_binary)
        morgenstern_search_from_binary (app->max, in, search_type_7, app);
      else
        morgenstern_search (app->max, in, search_type_7, app);
    }
  else
    {
      if (app->in_binary)
        {
          if (app->inmem)
            morgenstern_search_dual_binary_mem (in, app->infile, search_type_7, app);
          else
            morgenstern_search_dual_binary (in, app->infile, search_type_7, app);
        }
      else
        {
          if (app->inmem)
            morgenstern_search_dual_mem (in, app->infile, search_type_7, app);
          else
            morgenstern_search_dual (in, app->infile, search_type_7, app);
        }
    }
  return 0;
}
...
{
  struct fv_app_morgenstern_search_type_7_t *app = (struct fv_app_morgenstern_search_type_7_t *) state->input;
  switch (key)
    {
    case 'm':
      app->inmem = 1;
...
{
  struct fv_app_morgenstern_search_type_7_t app;
