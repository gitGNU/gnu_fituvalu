...
static void
search_type_6 (unsigned long long m1, unsigned long long n1, unsigned long long m2, unsigned long long n2, FILE *out)
{
...
static int
morgenstern_search_type_6 (FILE *in, FILE *out)
{
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      mpz_init (a[i][j]);
  mpz_inits (x1, _y1, z1, m12, n12, x2, y2, z2, m22, n22,
             yx1dif, yx1sum, yx2dif, yx2sum,
             NULL);
  small_morgenstern_search (max, in, search_type_6, out);
...
main (int argc, char **argv)
{
  ...
  return morgenstern_search_type_6 (stdin, stdout);
}
