...
struct fv_app_small_morgenstern_search_type_6_t;

static int
no_filter (struct fv_app_small_morgenstern_search_type_6_t *app, int num)
{
  return 1;
}
...
struct fv_app_small_morgenstern_search_type_6_t
{
  ...
  int (*filter_square) (struct fv_app_small_morgenstern_search_type_6_t *, int);
  FILE *out;
};

static int
filter (struct fv_app_small_morgenstern_search_type_6_t *app, int num_squares)
{
  int count = 5;
...
static void
search_type_6 (unsigned long long m1, unsigned long long n1, unsigned long long m2, unsigned long long n2, void *data)
{
  struct fv_app_small_morgenstern_search_type_6_t *app = (struct fv_app_small_morgenstern_search_type_6_t *) data;
...
*/
  ...
  // A = YX1sum * Z2;
  // H = Yx1sum * YX2sum;
  mpz_mul (app->a[2][0], app->z1, app->yx2dif);
  mpz_mul (app->a[2][0], app->a[2][0], app->a[2][0]);

  mpz_mul (app->a[0][1], app->yx1dif, app->yx2dif);
  mpz_mul (app->a[0][1], app->a[0][1], app->a[0][1]);

  mpz_mul (app->a[1][2], app->yx1sum, app->yx2dif);
  ...
  // C^2 = H^2 + G^2 - F^2;
  // E^2 = C^2 + I^2 - D^2;
  mpz_add (app->a[2][2], app->a[0][1], app->a[0][0]);
  mpz_sub (app->a[2][2], app->a[2][2], app->a[1][2]);

  mpz_add (app->a[1][0], app->a[0][1], app->a[2][1]);
  mpz_sub (app->a[1][0], app->a[1][0], app->a[1][2]);

  mpz_add (app->a[0][2], app->a[2][1], app->a[2][0]);
  mpz_sub (app->a[0][2], app->a[0][2], app->a[1][2]);

  mpz_add (app->a[1][1], app->a[0][2], app->a[2][2]);
  mpz_sub (app->a[1][1], app->a[1][1], app->a[1][0]);
...
int
fituvalu_morgenstern_search_type_6 (struct fv_app_small_morgenstern_search_type_6_t *app, FILE *in)
{
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      mpz_init (app->a[i][j]);
  mpz_inits (app->x1, app->_y1, app->z1, app->m12, app->n12, app->x2, app->y2, app->z2, app->m22, app->n22,
             ...
             NULL);
  small_morgenstern_search (app->max, in, search_type_6, app);
...
{
  struct fv_app_small_morgenstern_search_type_6_t *app = (struct fv_app_small_morgenstern_search_type_6_t *) state->input;
  switch (key)
    {
    case 'f':
...
{
  struct fv_app_small_morgenstern_search_type_6_t app;
  memset (&app, 0, sizeof (app));
  app.display_square = display_square_record;
  app.filter_square = no_filter;
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_morgenstern_search_type_6 (&app, stdin);
}
