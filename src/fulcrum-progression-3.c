...
struct fv_app_fulcrum_progression3_t
{
...
{
  struct fv_app_fulcrum_progression3_t *app = (struct fv_app_fulcrum_progression3_t *) data;
...
{
  struct fv_app_fulcrum_progression3_t *app = (struct fv_app_fulcrum_progression3_t *) state->input;
  char *end = NULL;
  switch (key)
    {
    case 'I':
...
int
fituvalu_fulcrum_progression3 (struct fv_app_fulcrum_progression3_t *app)
{
  if (app->instream && !app->in_binary)
    ...
    fwd_4sq_progression1 (app->oneshot, app->start, app->finish, app->incr,
...
{
  struct fv_app_fulcrum_progression3_t app;
  memset (&app, 0, sizeof (app));
  app.incr = 1;
  app.display_record = display_nine_record;
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_fulcrum_progression3 (&app);
}
