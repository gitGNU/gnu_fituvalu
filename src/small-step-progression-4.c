...
struct fv_app_small_step_progession4_t
{
  FILE *instream;
...
{
  struct fv_app_small_step_progession4_t *app = (struct fv_app_small_step_progession4_t *) data;
//|--+--*--*----------------*--+--*----------------*--*--+--|
//      ^  ^                ^     ^
//      1  2                3     4
  progression[1] = one;
  ...
  unsigned long long d1 = progression[2] - progression[1];
...
{
  struct fv_app_small_step_progession4_t *app = (struct fv_app_small_step_progession4_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_step_progression4 (struct fv_app_small_step_progession4_t *app)
{
  if (app->instream)
  ...
  if (key == ARGP_KEY_HELP_POST_DOC)
    {
      char *s =
        ...
        "        ^  ^        ^     ^\n"
        "        1  2        3     4\n"
        "We iterate over 1, 3, and 4, and 2 is at half of 4-3 + 1.";
      char *new_text = NULL;
...
{
  struct fv_app_small_step_progession4_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_step_progression4 (&app);
}
