...
struct fv_app_small_reverse_step_progression1_t
{
  FILE *instream;
...
{
  struct fv_app_small_reverse_step_progression1_t *app = (struct fv_app_small_reverse_step_progression1_t *) data;
  progression[0] = one;
  progression[1] = two;
  progression[2] = three;
  progression[3] = four;
  int count = 4;
  long long amt1 = progression[1] - progression[0];
  long long amt2 = progression[3] - progression[2];
  progression[4] = progression[3] + amt1;
...
{
  struct fv_app_small_reverse_step_progression1_t *app = (struct fv_app_small_reverse_step_progression1_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_reverse_step_progression1 (struct fv_app_small_reverse_step_progression1_t *app)
{
  if (app->instream)
    small_read_square_and_run (app->instream, small_rev_4sq_progression1,
                               check_progression, app->start, app->finish, app);
  else
    small_loop_and_run (small_rev_4sq_progression1, check_progression,
...
{
  struct fv_app_small_reverse_step_progression1_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_reverse_step_progression1 (&app);
}
