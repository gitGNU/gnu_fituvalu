...
struct fv_app_small_fulcrum_progression3_t
{
  FILE *instream;
...
{
  struct fv_app_small_fulcrum_progression3_t *app = (struct fv_app_small_fulcrum_progression3_t *) data;
  progression[3] = one;
  progression[4] = two;
  progression[5] = three;
  progression[6] = four;
  if (progression[5] == progression[6])
    return;
  int count = 4;
  unsigned long long e1 = progression[4] - progression[3];
  unsigned long long f1 = progression[6] - progression[5];
  unsigned long long d1 = e1 + f1;
  progression[7] = progression[6] + e1;
  progression[8] = progression[7] + d1;
  progression[2] = progression[3] - f1;
...
{
  struct fv_app_small_fulcrum_progression3_t *app = (struct fv_app_small_fulcrum_progression3_t *) state->input;
  switch (key)
    {
    case 'i':
      if (strcmp (arg, "-") == 0)
...
int
fituvalu_small_fulcrum_progression3 (struct fv_app_small_fulcrum_progression3_t *app)
{
  if (app->instream)
...
{
  struct fv_app_small_fulcrum_progression3_t app;
  memset (&app, 0, sizeof (app));
  app.out = stdout;
  argp_parse (&argp, argc, argv, 0, 0, &app);
  return fituvalu_small_fulcrum_progression3 (&app);
